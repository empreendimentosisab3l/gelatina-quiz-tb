...
const options = [
  { emoji: '😴', label: 'Menos de 5h' },
  { emoji: '🌙', label: '5-7h' },
  { emoji: '💤', label: '7-9h' },
  { emoji: '😊', label: 'Mais de 9h' },
]

export default function SleepStep({ onNext, onChange }) {
  const [selected, setSelected] = useState(null)

  const handleSelect = (option) => {
    setSelected(option.label)
    onChange('sleep', option.label)
  ...
  return (
    <div className="flex flex-col items-center pt-6 pb-8 flex-1">
      <h2 className="text-2xl font-bold text-gray-800 text-center mb-2">
        Quantas horas voce <span className="text-purple-600">dorme por noite</span>?
      </h2>
      <p className="text-muted-foreground text-center mb-6">
        O sono influencia diretamente no emagrecimento
      </p>
