...
const options = [
  { emoji: '😢', label: 'Nao estou feliz' },
  { emoji: '😐', label: 'Poderia ser melhor' },
  { emoji: '💪', label: 'Estou trabalhando nisso' },
]

export default function AppearanceStep({ onNext, onChange }) {
  const [selected, setSelected] = useState(null)

  const handleSelect = (option) => {
    setSelected(option.label)
    onChange('appearance', option.label)
  ...
  return (
    <div className="flex flex-col items-center pt-6 pb-8 flex-1">
      <h2 className="text-2xl font-bold text-gray-800 text-center mb-2">
        Voce esta feliz com{' '}
        <span className="text-purple-600">sua aparencia atual</span>?
      </h2>
